...
export default function PdfReader({ 
  ...
  useEffect(() => {
    if (activeTranslationText && selectionRects.length > 0 && !selection) {
  ...
  }, [savedVocab, activeTranslationText, selectionRects.length, selection]);
  ...
  return (
    ...
    >
      ...
      {numPages !== null && (
        <div className="h-16 bg-slate-800 border-b border-slate-700 flex items-center px-6 shrink-0 z-50 relative">
          ...
          >
                ...
                <span className="hidden lg:block">{isSplitMode ? "Exit Split" : "Split Mode"}</span>
          </button>
      ...
      <div className={`flex-1 flex overflow-hidden relative ${isSplitMode ? 'flex-row' : ''}`}>
        {/* Desk Surface (PDF Scroll Area) */}
        <div 
          ref={containerRef}
          onMouseUp={handleMouseUp}
          className={`flex-1 overflow-y-auto overflow-x-hidden bg-[#1a1c1e] custom-scrollbar relative p-8 md:p-12 selection:bg-indigo-500/30 ${isResizing ? '' : 'transition-all duration-500'}`}
          style={{ 
            backgroundImage: 'radial-gradient(circle at center, #23272b 0%, #1a1c1e 100%)',
            flex: isSplitMode ? `1 1 calc(100% - ${sidebarWidth}px)` : '1 1 100%'
          }}
        >
        ...
        <AnimatePresence>
            {isSidebarOpen && (
              <>
                {/* Resizer Divider Bar - Only in Split Mode */}
                {isSplitMode && (
                  <div 
                    onMouseDown={(e) => {
                      e.preventDefault();
                    ...
                    className="w-1.5 bg-slate-800 hover:bg-indigo-500/50 cursor-col-resize transition-colors z-[215] flex items-center justify-center group relative"
                  >
                    <div className="w-[2px] h-8 bg-slate-600 group-hover:bg-indigo-400 rounded-full" />
                    {/* Visual Hover effect */}
                    <div className="absolute inset-y-0 -left-2 -right-2 hidden group-hover:block" />
                  </div>
                )}

                <motion.div 
                  initial={isSplitMode ? { width: 0 } : { x: '100%' }}
                  animate={isSplitMode ? { width: sidebarWidth } : { x: 0 }}
                  exit={isSplitMode ? { width: 0 } : { x: '100%' }}
                  transition={{ type: 'spring', damping: 25, stiffness: 200 }}
                  className={`${isSplitMode ? 'relative overflow-hidden' : 'absolute right-0 top-0 bottom-0 w-[85%] sm:w-[350px] lg:w-[400px]'} h-full z-[210] shadow-[-10px_0_30px_rgba(0,0,0,0.2)] bg-white`}
                  style={isSplitMode ? { width: sidebarWidth } : {}}
                >
                    <div className="relative h-full">
                       {/* Close handle handle */}
                       {!isSplitMode && (
