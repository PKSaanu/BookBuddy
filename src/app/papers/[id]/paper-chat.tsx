...
export default function PaperChat({ paperTitle, paperAuthors, isSidebar = false, preferredLanguage = 'Tamil', onClose }: PaperChatProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: `Hello! I'm your study companion for "${paperTitle}". I can help you with paperTranslations or answer questions about the paper's content. What's on your mind?`
    }
  ]);
    ...
    try {
      ...
      if (!aiMsgContent || aiMsgContent.trim() === "") {
        const apologyMsg: Message = { 
          role: 'assistant', 
          content: "I'm sorry, I'm finding it hard to phrase my response for this part of the paper. Could you try asking in a slightly different way?" 
        };
  ...
  return (
    <div className={`flex flex-col h-full bg-slate-50 border-l border-slate-200 overflow-hidden ${isSidebar ? 'border-none' : ''}`}>

      {/* Header */}
      <div className="p-5 border-b border-slate-100 bg-white shrink-0 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-[13px] font-bold text-[#10175b] tracking-tight">Paper Buddy AI</h3>
      ...
      >
        ...
        {isLoading && (
          ...
          >
            <div className="bg-white border border-slate-100 text-slate-400 p-4 rounded-[24px] rounded-tl-none flex items-center gap-3 italic text-[13px] shadow-sm">
      ...
      <div className="p-4 bg-white shrink-0 border-t border-slate-100">
        <form
          onSubmit={handleSend}
          className="flex items-end gap-2 border border-slate-200 rounded-lg p-1 focus-within:border-[#10175b] focus-within:ring-1 focus-within:ring-[#10175b] transition-all bg-slate-50"
        >
          <textarea
            rows={1}
            value={input}
