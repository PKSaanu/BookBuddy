                        ...
                        {recentItem ? (
                            <>
                                <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
                                    {/* Huge Left Item Card */}
                                    <Link href={recentItem.type === 'book' ? `/books/${recentItem.id}` : `/papers/${recentItem.id}`} className="xl:col-span-2 group">
                                        <div className="bg-slate-950 rounded-[2rem] h-[500px] relative overflow-hidden isolate transform-gpu backface-hidden shadow-xl border border-slate-200/20 group-hover:shadow-2xl transition-all duration-500 group-hover:scale-[1.01] will-change-transform">
                                            ...
                                            <div className={`absolute inset-0 bg-gradient-to-tr ${recentItem.type === 'book' ? 'from-[#0a0d2e] to-[#0a0d2e]/40' : 'from-rose-950 to-rose-950/40'} opacity-80 rounded-[2rem]`} />
                                            <div className={`absolute inset-x-0 bottom-0 h-2/3 bg-gradient-to-t ${recentItem.type === 'book' ? 'from-[#0a0d2e] via-[#0a0d2e]/60' : 'from-rose-950 via-rose-950/60'} to-transparent z-10 rounded-[2rem]`} />

                                            <div className="absolute bottom-0 left-0 p-12 w-full z-20">
                                                <div className="flex items-center gap-4 mb-6">
                                                    <div className={`${recentItem.type === 'book' ? 'bg-[#0f766e]' : 'bg-rose-700'} text-white text-[11px] font-bold uppercase tracking-[0.1em] px-4 py-2 rounded-full shadow-sm`}>
                                                        Recently Added {recentItem.type === 'paper' ? 'Research' : ''}
                                                    </div>
                                                    {recentItem.type === 'book' && recentItem.totalPages && (
                                                ...
                                                <h3 className="text-4xl md:text-5xl font-serif font-bold text-white leading-tight mb-3 drop-shadow-2xl line-clamp-2">
                                                    {recentItem.title}
                                                </h3>
                                                <p className="text-xl text-slate-300 font-medium drop-shadow-lg italic">{recentItem.author || 'Unknown Author'}</p>
                                            </div>
                                        </div>
                                    </Link>
                                ...
                                {oldItems.length > 0 && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12 pb-12">
                                        <h3 className="col-span-full font-bold text-slate-400 uppercase tracking-widest text-[11px] mb-2">Previous Collection</h3>
                                        {oldItems.map((item, index) => (
                                            <Link href={item.type === 'book' ? `/books/${item.id}` : `/papers/${item.id}`} key={item.id} className="group">
                                                <div className={`relative h-56 rounded-[24px] p-8 overflow-hidden isolate shadow-sm hover:shadow-xl transition-all duration-500 group-hover:scale-[1.02] group-hover:-translate-y-1 transform-gpu`}>
                                                    ...
                                                    <div className={`absolute inset-0 bg-gradient-to-br ${item.type === 'book' ? coverBackgrounds[index % coverBackgrounds.length] : 'from-rose-900 to-rose-700'} transition-transform duration-700 group-hover:scale-110`} />
                                                    ...
                                                    <div className="relative z-10 h-full flex flex-col justify-end">
                                                        <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-xl">
                                                            <div className="flex items-center gap-2 mb-1">
                                                                {item.type === 'paper' && <span className="bg-white/20 text-white text-[8px] font-black uppercase px-2 py-0.5 rounded-full backdrop-blur-sm">Research</span>}
